...
const rememberSchema = Type.Object({
  ...
  scope: Type.Union([Type.Literal("global"), Type.Literal("session")], {
    description: "global stores stable cross-session memory; session stores memory only for the current chat session.",
  }),
...
const recallSchema = Type.Object({
  ...
  scope: Type.Optional(
    Type.Union([Type.Literal("seed"), Type.Literal("global"), Type.Literal("session")], {
...
export function memoryTools(memory: MemoryStore): AgentTool[] {
  const remember: AgentTool<typeof rememberSchema> = {
    name: "remember",
    label: "Remember",
    description: "Store a concise memory note. Use scope `global` for stable cross-session facts/preferences, or `session` for context that should only apply to the current chat session.",
  ...
  const recall: AgentTool<typeof recallSchema> = {
    name: "recall",
    label: "Recall",
    description: "Read memory by optional keyword query and/or scope. Use scope `seed` for profile seed memory, `global` for cross-session memory, or `session` for the current chat session memory.",
